...
DriveSubsystem::DriveSubsystem() :
        ...
        m_ticksPerInch("Ticks Per Inch", (33540/(6*3.14159265358979323))),
		m_balanceMaxSpeed("Max speed of Balance", 0.7),
		m_balanceCalibration("Robot Pitch", 0.8),
		m_balanceMaxCalibration("max proportion of robot pitch", 0.4),
		m_driveSpeedModifier("Drive speed Modifier", 0.75),
		m_driveSpeedModifierSlow("Drive speed Modifier slowed", 0.25),
		m_balanceDistInch("Distance in inches to move to balance", -55),
		m_autoBalanceMaxSpeed("Max speed of Balance IN AUTON", 0.28),
		m_autoBalanceDeadband("Deadband of Auton position", 1000),
...
void DriveSubsystem::teleopInit() {
	...
	m_startingEncoderValueBalance = 0;
	m_targetEncoderValueBalance = 0;
...
void DriveSubsystem::teleop() {
	...
	SmartDashboard::PutNumber("its the pitch",ahrs->GetPitch());
	SmartDashboard::PutNumber("its the roll",ahrs->GetRoll());
...
void DriveSubsystem::Balance(){
	...
	if (abs(m_currentPitch) >= 1.6){
		...
		if(m_targetEncoderValueBalance == 0){
			setMotorSpeed(motorSpeed, motorSpeed);
		} else if (abs(m_currentEncoderValueBalance) < abs(m_targetEncoderValueBalance - m_autoBalanceDeadband.Get())){
			setMotorSpeed(m_autoBalanceMaxSpeed.Get(), m_autoBalanceMaxSpeed.Get());
		} else {
			setMotorSpeed(0,0);
			if ((abs(m_currentPitch) >= 1.6))
			{
...
void DriveSubsystem::BalanceInit(){
	m_startingEncoderValueBalance = getRobotPosition();
	m_targetEncoderValueBalance = m_startingEncoderValueBalance + (m_balanceDistInch.Get() * m_ticksPerInch.Get());
}